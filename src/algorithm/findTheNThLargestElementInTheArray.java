...
public class findTheNThLargestElementInTheArray {
     ...
     * <p>
     * The traditional method, using the sorting algorithm,
     * and then taking out the corresponding values,
     * the time complexity is nlogn level , we're going to use three quick sort here
     *
     * @param array 目标数组
     * @param n     需求的大小排名
     * @return 第N大的元素
    ...
    /**
     * 使用快速排序的思路来解决,每次partition的时候比较一下元素的位置,然后选择需要那一部分继续快速找值
     * Use a quick sort of thinking to solve,
     * compare the location of the element each time partition,
     * and then choose to need that part to continue to find the value quickly
     * 算法时间期望值为 n+n/2+n/4+n/8......+1 = 2n 时间复杂度是N级别的
     *
     * @param arr
     * @param n
     * @return
