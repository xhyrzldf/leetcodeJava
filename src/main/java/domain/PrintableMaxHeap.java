...
public class PrintableMaxHeap extends MaxHeap<Comparable<Integer>> {
    ...
    public void treePrint() {
        ...
        for (int i = 1; i <= size(); i++) {
            ...
            assert (Integer) elementData[i] >= 0 && (Integer) elementData[i] < 100;
        ...
        for (int level = 0; level < maxLevel; level++) {
            ...
            for (int indexCurLevel = 0; indexCurLevel < curLevelNumber; index++, indexCurLevel++) {
                line1 =
                        putNumberInLine(
                                (Integer) elementData[index],
                                line1,
                                indexCurLevel,
                                curTreeMaxLevelNumber * 3 - 1,
                                isLeft);
                isLeft = !isLeft;
            }
    ...
    private String putNumberInLine(
            Integer num, String line, int indexCurLevel, int curTreeWidth, boolean isLeft) {
        ...
        if (num >= 10)
            ...
            else line = line.substring(0, offset + 1) + num.toString() + line.substring(offset + 2);
        }
        return line;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N; i++) maxHeap.add((int) (Math.random() * M));
        maxHeap.treePrint();
    }
}
